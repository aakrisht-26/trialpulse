...
**Finding for the prompt work.** A count-only check found that 2 test texts appear word for word among the example phrases in `docs/labeling_guide.md` (generic phrases; the guide predates the sample). The LLM prompt therefore cannot copy every guide example verbatim; the guard names any leak by hash prefix only. A scan of all 70 tracked text files with the guard found test texts in only two, both written before the sample existed: the guide (2) and one synthetic string in `tests/nlp/test_gold.py` (1). Neither is a prompt file, and none of the docs written in this run contains a test text.
