...
- **ADR 0008, ClinicalTrials.gov attribution** (docs/adr/0008-clinicaltrials-gov-attribution.md): following the ClinicalTrials.gov terms and conditions, the README, the data card (Step 12) and every dashboard page (Step 16) show the source attribution (ClinicalTrials.gov), a 'data as of' date (the date ClinicalTrials.gov processed the data) and a note that TrialPulse normalizes the records and adds derived risk scores.
- **ADR 0009, test lock published on origin** (docs/adr/0009-test-lock-requires-published-tag.md): in addition to ADR 0004, the prereg-v1 tag must exist on origin as the same annotated tag object, pointing at the same commit (checked with git ls-remote), so the registration is public before any locked result exists.
- **ADR 0010, reference labels** (docs/adr/0010-reference-labels-from-an-adjudicated-model-panel.md): the Section 11 gold set is not labeled by hand. It holds reference labels from an adjudicated model panel: three independent labelers per text that see only the labeling guide and the text, a fourth labeler for splits, and a fresh labeler relabeling a seeded 10% for a consistency check. It is always described with that wording, never as human-labeled. Any model graded against these labels must come from a different model family than the panel. Test labels are committed before any LLM prompt work, prompt work uses only the dev items, and a test checks that no test text appears in the prompt files. The labeling app becomes an optional review tool.
